...
class CompizHelper():
    ...
    def set_wallpapers_from_array(self, wallpapers):
        self.context.Plugins['wallpaper'].Screen['bg_color2'].Value = [[0, 0, 0, 65535]] * len(wallpapers)
        self.context.Plugins['wallpaper'].Screen['bg_color1'].Value = [[0, 0, 0, 65535]] * len(wallpapers)
        self.context.Plugins['wallpaper'].Screen['bg_image'].Value = wallpapers
        self.context.Plugins['wallpaper'].Screen['bg_image_pos'].Value = [0] * len(wallpapers)
        self.context.Plugins['wallpaper'].Screen['bg_fill_type'].Value = [0] * len(wallpapers)
    ...
    def set_wallpapers_from_folder(self, path, reverse=False):
        ...
        if os.path.exists(path) and os.path.isdir(path):
            for file in os.listdir(path):
                ...
                if os.path.isfile(abs_path) and os.path.splitext(file)[1].lower() in ['.jpg', '.png']:
    ...
    def set_wallpapers_from_folder_ordered(self, path, reverse=False):
        ...
        wallpapers.sort(reverse=reverse)
        self.set_wallpapers_from_array([wallpaper.file for wallpaper in wallpapers])
        return wallpapers

    def list_wallpapers(self):
        return self.context.Plugins['wallpaper'].Screen['bg_image'].Value

    def sort_wallpapers(self, reverse=False):
        wallpapers = []
        for file_path in self.list_wallpapers():
            if os.path.isfile(file_path) and os.path.splitext(file_path)[1].lower() in ['.jpg', '.png']:
...
if __name__ == "__main__":
    ...
    parser.add_argument('action', type=str, help="Action to execute. Available actions: set, order_set, sort")
    parser.add_argument('path', type=str, help="Path to directory with images")
    parser.add_argument('-r', '--reverse', action="store_true", help="Order image directly or reverse ordered")
    ...
    if args.action == "set":
        ...
        for wallpaper in ch.set_wallpapers_from_folder(args.path, args.reverse):
            print "\t%s" % wallpaper
    elif args.action == "order_set":
        print "New wallpaper set:"
        for wallpaper in ch.set_wallpapers_from_folder_ordered(args.path, args.reverse):
            print "\t%s" % wallpaper
