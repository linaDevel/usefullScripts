...
def create_environment():
    ...
    data = {"name": cluster_settings['env_name'], "release_id": release_id,
    ...
    for option in settings:
        section = False
        if option in ('sahara', 'ceilometer'):
    ...
    while True:
        ...
        actual_kvm_count = len(
            [
             k for k in client.list_nodes()
             if not k['cluster'] and k['online'] and k['status'] == 'discover'
            ]
        )
        ...
        counter += 5
        if counter > 1200:
            raise RuntimeError("Timeout waiting for nodes to come online.\n"
                               "# of nodes that successfully came up: %s"
                               % actual_kvm_count)
        time.sleep(5)
...
def update_node_networks(client, node_id, interfaces_dict, raw_data=None):
    ...
    for interface in interfaces:
        ...
        for i in interfaces_dict.get(name, []):
            if i in all_networks:
                interface['assigned_networks'].append(all_networks[i])
...
def generate_network_config():
    ...
    management_vlan = cluster_settings.get("management_vlan", None)
    if management_vlan:
        management_vlan = int(management_vlan)
    storage_vlan = cluster_settings.get("storage_vlan", None)
    if storage_vlan:
        storage_vlan = int(storage_vlan)
    private_vlan = cluster_settings.get("private_vlan", None)
    if private_vlan:
        private_vlan = int(private_vlan)

    networks["public"] = {"ip_ranges": [["%s" % public_start,
                          "%s" % public_end]],
                          "cidr": "%s" % cidr,
                          "gateway": cluster_settings.get("gateway")}
    networks["management"] = {"vlan_start": management_vlan}
    networks["storage"] = {"vlan_start": storage_vlan}
    networks["networking_parameters"] = {
        "floating_ranges": [["%s" % floating_start, "%s" % floating_end]]}

    if cluster_settings.get("net_segment_type") == "vlan":
        floating_vlan_start = cluster_settings.get(
            "floating_vlan_range").split('-')[0]
        floating_vlan_end = cluster_settings.get(
            "floating_vlan_range").split('-')[1]
        networks["networking_parameters"]["vlan_range"] = [
            int(floating_vlan_start), int(floating_vlan_end)]
    else:
        networks["private"] = {"vlan_start": private_vlan}
    if cluster_settings.get("net_provider") == "nova_network":
        nn_floating_vlan = cluster_settings.get("nn_floating_vlan")
        networks["networking_parameters"][
            "fixed_networks_vlan_start"] = int(nn_floating_vlan)
